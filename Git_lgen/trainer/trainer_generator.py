...
class LayGenerator(object):
    ...
    def train(self):
        ...
        if cfg.TRAIN.EVALUATOR != '':
            layout_evaluator.load_state_dict(torch.load(cfg.TRAIN.EVALUATOR, map_location=torch.device('cpu')))
        if cfg.TRAIN.GENERATOR_MODEL != '':
            layout_generator.load_state_dict(torch.load(cfg.TRAIN.GENERATOR_MODEL, map_location=torch.device('cpu')))
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step, data in enumerate(self.dataloader_train, 0):
                ...
                refine_layout = torch.cat((layout_init_contour, refine_layout_room), dim=0) 
                ...
                if step == 299:
                    ...
                    gt_layout = torch.cat((layout_init_contour, gt_layout_room), dim=0) 
    ...
    def evaluate(self):
        ...
        if cfg.EVAL.MODEL_EVALUATOR == '':
            ...
            eval_layout_evaluator.load_state_dict(torch.load(self.eval_model_path, map_location=torch.device('cpu')))
            eval_layout_generator.load_state_dict(torch.load(self.gen_model_path, map_location=torch.device('cpu')))
        eval_layout_evaluator 
        eval_layout_generator 

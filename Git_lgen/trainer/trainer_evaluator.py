...
class LayoutTrainer(object):
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step, data in enumerate(self.dataloader_train, 0):
                ...
                layout1 = torch.cat((layout1_init_contour, layout1_room), dim=1).transpose(0, 1)  
                layout2 = torch.cat((layout2_init_contour, layout2_room), dim=1).transpose(0, 1)  
                pred_score1 = layout_evaluator(layout1)
                pred_score2 = layout_evaluator(layout2)
                # indicators_values = torch.from_numpy(np.array(indicator_values[0])).type(torch.float32)  
                indicator_values = indicator_values[0].reshape([len(indicator_values[0]), 1]).float()  
            ...
            with torch.no_grad():
                ...
                for step, data in enumerate(self.dataloader_test, 0):
                    ...
                    test_layout1 = torch.cat((test_layout1_init_contour, test_layout1_room), dim=1).transpose(0, 1)  
                    test_layout2 = torch.cat((test_layout2_init_contour, test_layout2_room), dim=1).transpose(0, 1)  
                    test_pred_score1 = layout_evaluator.forward(test_layout1)
                    test_pred_score2 = layout_evaluator.forward(test_layout2)
                    test_indicator_values = test_indicator_values[0].reshape([len(test_indicator_values[0]), 1]).float()  
    ...
    def evaluate(self):
        ...
        if cfg.EVAL.MODEL_EVALUATOR == '':
            ...
            eval_layout_evaluator.load_state_dict(torch.load(self.model_path,  map_location=torch.device('cpu')))
        eval_layout_evaluator  
        ...
        with open(save_score_path, "w") as score_file:
            for step, data in enumerate(self.dataloader_test, 0):
                ...
                eval_layout1_init_contour, eval_layout2_init_contour = eval_init_contour[0], \
                                                                       eval_init_contour[0]
                eval_layout1 = torch.cat((eval_layout1_init_contour, eval_layout1_room), dim=1).transpose(0, 1)  
                eval_layout2 = torch.cat((eval_layout2_init_contour, eval_layout2_room), dim=1).transpose(0, 1)  
